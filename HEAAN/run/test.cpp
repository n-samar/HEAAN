...
int main(int argc, char **argv) {
    ...
    if (string(argv[1]) == "EncryptBySk")
        TestScheme::testEncryptBySk(logq, logp, logn);
    if (string(argv[1]) == "DecryptForShare")
        TestScheme::testDecryptForShare(logq, logp, logn, stol(argv[2]));
    ...
    logq = logp + 10;  //< suppose the input ciphertext of bootstrapping has
                       // logq = logp + 10
    logn = 3;          //< larger logn will make bootstrapping tech much slower
    long logT = 4;     //< this means that we use Taylor approximation in
                       //[-1/T,1/T] with double angle fomula
