...
void Scheme::encryptBySk(Ciphertext& cipher, SecretKey& secretKey,
                         complex<double>* vals, long n, long logp, long logq,
                         double sigma1) {
    Plaintext plain;
    encode(plain, vals, n, logp, logq);
...
void Scheme::encryptBySk(Ciphertext& cipher, SecretKey& secretKey, double* vals,
                         long n, long logp, long logq, double sigma1) {
    Plaintext plain;
    encode(plain, vals, n, logp, logq);
...
complex<double>* Scheme::decryptForShare(SecretKey& secretKey,
                                         Ciphertext& cipher, long logErrBound) {
    Plaintext plain;
    decryptMsg(plain, secretKey, cipher);
...
void Scheme::addConstAndEqual(Ciphertext& cipher, complex<double> cnst,
    ...
    ZZ q = ring.qpows[cipher.logq];
    ZZ cnstrZZ = logp < 0
                     ? EvaluatorUtils::scaleUpToZZ(cnst.real(), cipher.logp)
                     : EvaluatorUtils::scaleUpToZZ(cnst.real(), logp);
    ZZ cnstiZZ = logp < 0
                     ? EvaluatorUtils::scaleUpToZZ(cnst.imag(), cipher.logp)
...
void Scheme::leftRotateFast(Ciphertext& res, Ciphertext& cipher, long r) {
    ...
    Key* key = isSerialized
                   ? SerializationUtils::readKey(serLeftRotKeyMap.at(r))
                   : leftRotKeyMap.at(r);
    res.copyParams(cipher);
...
void Scheme::evalExpAndEqual(Ciphertext& cipher, long logT, long logI) {
    ...
    if (logSlots < logNh) {
        ...
        exp2piAndEqual(
            cipher, bootContext->logp);  // bitDown: logT + 1 + 3(logq + logI)
        ...
        exp2piAndEqual(
            c2, bootContext->logp);  // c2 bitDown: logT + 1 + 3(logq + logI)
        for (long i = 0; i < logI + logT; ++i) {
            squareAndEqual(c2);
            squareAndEqual(cipher);
