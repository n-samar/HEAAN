...
void TestScheme::testDecryptForShare(long logq, long logp, long logn,
                                     long logErrorBound) {
  ...
  complex<double>* dvecShare =
      scheme.decryptForShare(secretKey, cipher, logErrorBound);
  complex<double>* dvec = scheme.decrypt(secretKey, cipher);
  ...
  for (long i = 0; i < n; ++i) {
    ...
    cout << "dec and decForShare error : " << i << " :"
         << (dvec[i] - dvecShare[i]) << endl;
    cout << "---------------------" << endl;
  }
