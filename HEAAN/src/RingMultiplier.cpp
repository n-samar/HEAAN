...
RingMultiplier::RingMultiplier() {
    ...
    for (long i = 0; i < nprimes; ++i) {
        ...
        for (long j = 0; j < N; ++j) {
            ...
            mulMod(scaledRootPows[i][jprime], scaledRootPows[i][jprime],
                   (1ULL << 32), pVec[i]);
            uint64_t rootpowInv = powerInv;
            mulMod(scaledRootInvPows[i][jprime], rootpowInv, (1ULL << 32),
                   pVec[i]);
    ...
    for (long i = 0; i < nprimes; ++i) {
        ...
        pProd[i] =
            (i == 0) ? to_ZZ((long)pVec[i]) : pProd[i - 1] * (long)pVec[i];
        pProdh[i] = pProd[i] / 2;
        pHat[i] = new ZZ[i + 1];
        ...
        for (long j = 0; j < i + 1; ++j) {
            ...
            coeffpinv_array[i][j] =
                PrepMulModPrecon(pHatInvModp[i][j], pVec[j]);
        }
    }
}
