    ...
    if (bootContextMap.find(logSlots) == bootContextMap.end()) {
            ...
            for (ki = 0; ki < slots; ki += k) {
                for (pos = ki; pos < ki + k; ++pos) {
                    ...
                    for (i = 0, jdx = Nh, idx = 0; i < dslots;
                         ++i, jdx += dgap, idx += dgap) {
                        pvec[idx] =
                            EvaluatorUtils::scaleUpToZZ(pvals[i].real(), logp);
                        pvec[jdx] =
                            EvaluatorUtils::scaleUpToZZ(pvals[i].imag(), logp);
                    }
                    bndvec[pos] = maxBits(pvec, N);
                    np = ceil((bndvec[pos] + logQ + 2 * logN + 2) /
                              (double)pbnd);
        ...
        for (ki = 0; ki < slots; ki += k) {
            for (pos = ki; pos < ki + k; ++pos) {
                ...
                np =
                    ceil((bndvecInv[pos] + logQ + 2 * logN + 2) / (double)pbnd);
        ...
        bootContextMap.insert(pair<long, BootContext*>(
            logSlots, new BootContext(rpvec, rpvecInv, rp1, rp2, bndvec,
                                      bndvecInv, bnd1, bnd2, logp)));
    }
}
